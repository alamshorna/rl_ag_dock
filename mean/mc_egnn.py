...
from utils import print_log, segment_softmax
...
class MC_E_GCL(nn.Module):
    ...
    def __init__(
            self,
            input_nf, # number of node features in the input h (per node)
            output_nf, # number of node features after this layer
            hidden_nf, # internal hidden dim used in MLPs for edges/nodes
            n_channel, # number of coordinate channels per node (so each node's coordinates are shape [n_channel, d])
            edges_in_d=0, # dimension of edge attributes (extra edge features besides geometric ones)
            act_fn=nn.SiLU(),
            residual=True, # add a residual connection at the node feature level
            attention=False, # if True, will compute attention weights on edges (gate messages)
            coords_agg='mean', 
            tanh=False,
            dropout=0.1):
        super(MC_E_GCL, self).__init__()
        input_edge = input_nf * 2 # for each edge i->j, the edge MLP will see both source node features + target node features concatenated
        ...
        self.edge_mlp = nn.Sequential(
            ...
            act_fn
        ) # this MLP produces edge features/messages i->j from the concat of node features + geometric info

        self.node_mlp = nn.Sequential(
            nn.Linear(hidden_nf + input_nf, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, output_nf)
        ) # given aggregated messages + current node state, compute updated node features
        ...
        if self.attention: # if attention=True, defines att_mlp = for each edge's feature vect, compute an attention weight a_ij that scales how much that edge's message contributes
    ...
    def edge_model(self, source, target, radial, edge_attr):
        '''
        compute edge features/messages for each edge in the graph
...
class MC_Att_L(nn.Module):
    ...
    def att_model(self, h, edge_index, radial, edge_attr):
        ...
        num_nodes = h.shape[0]  # number of nodes in this graph
        alpha = segment_softmax(alpha, row, num_segments=num_nodes) # [n_edge]
